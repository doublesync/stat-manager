...
def calculateTotalSpent(player) -> list[int]:
    ...
    for receipt in cashReceipts:
        totalEarned += receipt.amount if not receipt.taken else 0
...
def spentFrivolity() -> dict[str, int]:
    ...
    for player in players:
        spentCount[f"{player.firstName} {player.lastName}"] = calculateTotalSpent(player)[0]
    # Now sort the 'spentCount' dictionary & limit it to the top ten
    spentCount = dict(sorted(spentCount.items(), key=lambda item: item[1], reverse=True))
...
def earnedFrivolity() -> dict[str, int]:
    ...
    earnedCount: dict = {}
    for player in players:
        earnedCount[f"{player.firstName} {player.lastName}"] = calculateTotalSpent(player)[1]
    # Now sort the 'earnedCount' dictionary & limit it to the top ten
    earnedCount = dict(sorted(earnedCount.items(), key=lambda item: item[1], reverse=True))
    earnedCount = dict(list(earnedCount.items())[:10])
    return earnedCount


def positionFrivolity() -> dict[str, int]:
    positions: any = BasketballPlayer.objects.values_list('position', flat=True).distinct()
    positionCount: dict = {}
    for position in positions:
        positionCount[position] = BasketballPlayer.objects.filter(position=position).count()
    return positionCount


def archetypeFrivolity() -> dict[str, int]:
    archetypes: any = BasketballPlayer.objects.values_list('archetype', flat=True).distinct()
    archetypeCount: dict = {}
    for archetype in archetypes:
        archetypeCount[archetype] = BasketballPlayer.objects.filter(archetype=archetype).count()
...
def heightFrivolity() -> dict[int, int]:
    ...
    for height in heights:
        ...
        heightCount[formattedHeight] = BasketballPlayer.objects.filter(height=height).count()
    return heightCount


def weightModelFrivolity() -> dict[int, int]:
    weightModels: any = BasketballPlayer.objects.values_list('weightModel', flat=True).distinct()
    weightModelCount: dict = {}
    for weightModel in weightModels:
        weightModelCount[weightModel] = BasketballPlayer.objects.filter(weightModel=weightModel).count()
